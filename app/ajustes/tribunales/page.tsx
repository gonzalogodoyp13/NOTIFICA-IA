...
export default function TribunalesPage() {
  ...
  const [formData, setFormData] = useState({ nombre: '', direccion: '', comuna: '' })
  ...
  const handleOpenModal = (tribunal?: Tribunal) => {
    if (tribunal) {
      setEditingTribunal(tribunal)
      setFormData({ nombre: tribunal.nombre, direccion: tribunal.direccion || '', comuna: tribunal.comuna || '' })
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTribunal ? `/api/tribunales/${editingTribunal.id}` : '/api/tribunales'
      const method = editingTribunal ? 'PUT' : 'POST'
      ...
      if (result.ok) {
        await fetchTribunales()
        handleCloseModal()
      } else {
        alert('Error: ' + (result.error || 'Error al guardar'))
      }
    } catch (error) {
      console.error('Error saving tribunal:', error)
      alert('Error al guardar el tribunal')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('¿Estás seguro de que deseas eliminar este tribunal?')) return

    try {
      const response = await fetch(`/api/tribunales/${id}`, { method: 'DELETE' })
      const result = await response.json()
      ...
      alert('Error al eliminar el tribunal')
    }
  }
      ...
      <main className="pt-20 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-semibold text-gray-900 mb-2">Gestionar Tribunales</h1>
              <p className="text-gray-600">Administra los tribunales del sistema</p>
            </div>
            <button
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-lg shadow-md border border-gray-200 p-8">
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm mt-2">Haz clic en "Agregar" para crear tu primer tribunal.</p>
              </div>
            </div>
          ) : (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {tribunales.map((tribunal) => (
                      <tr key={tribunal.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{tribunal.id}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{tribunal.nombre}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tribunal.direccion || '-'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tribunal.comuna || '-'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button onClick={() => handleOpenModal(tribunal)} className="text-blue-600 hover:text-blue-900 mr-4">Editar</button>
                          <button onClick={() => handleDelete(tribunal.id)} className="text-red-600 hover:text-red-900">Eliminar</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {showModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
                <div className="p-6">
                  <h2 className="text-2xl font-semibold text-gray-900 mb-4">{editingTribunal ? 'Editar Tribunal' : 'Nuevo Tribunal'}</h2>
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end gap-3">
                      ...
                      <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50" disabled={submitting}>
                        {submitting ? 'Guardando...' : editingTribunal ? 'Actualizar' : 'Crear'}
                      </button>
                    </div>
                  </form>
                </div>
